...
if individual == local.p1 or individual == local.p2:
    if personal[0] < D < personal[1]:
        ...
        N = S[0] * (personal[1] - personal[0]) + S[1] * (D - personal[1])
    elif personal[2] < D < personal[3]:
        N = S[0] * (personal[1] - personal[0]) + S[1] * (personal[2] - personal[1]) + S[2] * (D - personal[2])
    elif personal[3] < D < personal[4]:
        N = S[0] * (personal[1] - personal[0]) + S[1] * (personal[2] - personal[1]) + S[2] * (
                personal[3] - personal[2]) + S[3] * (D - personal[3])
    elif personal[4] < D < personal[5]:
        N = S[0] * (personal[1] - personal[0]) + S[1] * (personal[2] - personal[1]) + S[2] * (
                personal[3] - personal[2]) + S[3] * (personal[4] - personal[3]) + S[4] * (D - personal[4])
    else:
        N = S[0] * (personal[1] - personal[0]) + S[1] * (personal[2] - personal[1]) + S[2] * (
                personal[3] - personal[2]) + S[3] * (personal[4] - personal[3]) + S[4] * (
                personal[5] - personal[4]) + S[5] * (D - personal[5])
    ...
    if family[0] < D < family[1]:
        ...
        N = S[0] * (family[2] - family[1]) + S[1] * (D - family[2])
    elif family[3] < D < family[4]:
        N = S[0] * (family[2] - family[1]) + S[1] * (family[3] - family[2]) + S[2] * (D - family[3])
    elif family[4] < D < family[5]:
        N = S[0] * (family[2] - family[1]) + S[1] * (family[3] - family[2]) + S[2] * (family[4] - family[3]) + S[3] * (
                D - family[4])
    elif family[5] < D < family[6]:
        N = S[0] * (family[2] - family[1]) + S[1] * (family[3] - family[2]) + S[2] * (family[4] - family[3]) + S[3] * (
                family[5] - family[4]) + S[4] * (D - family[5])
    else:
        N = S[0] * (family[2] - family[1]) + S[1] * (family[3] - family[2]) + S[2] * (family[4] - family[3]) + S[3] * (
                family[4] - family[4]) + S[4] * (family[6] - family[5]) + S[5] * (D - family[6])

elif individual == local.sp1 or individual == local.sp2:
    if singleparent[0] < D < singleparent[1]:
        N = S[0] * (D - singleparent[0])
    elif singleparent[1] < D < singleparent[2]:
        N = S[0] * (singleparent[1] - singleparent[0]) + S[1] * (D - singleparent[1])
    elif singleparent[2] < D < singleparent[3]:
        N = S[0] * (singleparent[1] - singleparent[0]) + S[1] * (singleparent[2] - singleparent[1]) + S[2] * (
                D - singleparent[2])
    elif singleparent[3] < D < singleparent[4]:
        N = S[0] * (singleparent[1] - singleparent[0]) + S[1] * (singleparent[2] - singleparent[1]) + S[2] * (
                singleparent[3] - singleparent[2]) + S[3] * (D - singleparent[3])
    elif singleparent[4] < D < singleparent[5]:
        N = S[0] * (singleparent[1] - singleparent[0]) + S[1] * (singleparent[2] - singleparent[1]) + S[2] * (
                singleparent[3] - singleparent[2]) + S[3] * (singleparent[4] - singleparent[3]) + S[4] * (
                D - singleparent[4])
    else:
        N = S[0] * (singleparent[1] - singleparent[0]) + S[1] * (singleparent[2] - singleparent[1]) + S[2] * (
                singleparent[3] - singleparent[2]) + S[3] * (singleparent[4] - singleparent[3]) + S[4] * (
                singleparent[5] - singleparent[4]) + S[5] * (D - singleparent[5])
